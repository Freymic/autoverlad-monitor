...
def fetch_all_data():
    ...
    # --- 2. LÖTSCHBERG LOGIK (Original API-Format) ---
    try:
        # Die von dir verifizierte URL (avwV2 statt avvV2)
        ...
        for item in l_res:
            ...
            if name:
                # Hier speichern wir das KOMPLETTE item-Objekt als JSON-String
                # Das entspricht exakt der API-Antwort für diese Station
                raw_api_data = json.dumps(item, ensure_ascii=False)
    ...
    # Standard-Keys sicherstellen
    for s in ["Kandersteg", "Goppenstein"]:
        if s not in results: results[s] = {"min": 0, "raw": "Keine Info"}

    save_to_db(results)
    return results

# Import für autoverlad_app.py (image_8d7123.png)
